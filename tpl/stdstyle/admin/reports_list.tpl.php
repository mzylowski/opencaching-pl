...
<?php foreach ($view->reports as $report) { ?>
    <tr>
      ...
      <td>
        <a href="<?=$report->getCache()->getCacheUrl()?>" class="links" rel="noopener" target="_blank"><img src="<?=$report->getCache()->getCacheIcon($view->user)?>" height=20 title="<?=tr($report->getCache()->getStatusTranslationKey())?>" alt="<?=tr($report->getCache()->getStatusTranslationKey())?>"> <?=$report->getCache()->getCacheName()?> (<?=$report->getCache()->getWaypointId()?>)</a><br>
        <?=$report->getCache()->getCacheLocationObj()->getLocationDesc(' &gt; ')?><br>
        <a href="<?=$report->getCache()->getOwner()->getProfileUrl()?>" class="links" rel="noopener" target="_blank"><?=$report->getCache()->getOwner()->getUserName()?></a>
      </td>
      <td class="<?=$report->getReportStyle()?>">
        ...
        <a href="<?=$report->getUserSubmit()->getProfileUrl()?>" class="links" rel="noopener" target="_blank"><?=$report->getUserSubmit()->getUserName()?></a>
      </td>
      <td class="<?=$report->getStatusClass()?>">
        ...
        <?php if ($report->getUserIdLastChange() != null) {?><br><a href="<?=$report->getUserLastChange()->getProfileUrl()?>" class="links" rel="noopener" target="_blank"><?=$report->getUserLastChange()->getUserName()?></a><?php }?>
      </td>
      <td><?php if ($report->getUserIdLeader() != null) { ?><a href="<?=$report->getUserLeader()->getProfileUrl()?>" class="links" rel="noopener" target="_blank"><?=$report->getUserLeader()->getUserName()?></a><?php }?></td>
    </tr>
<?php } ?>
  </table>
